...
- `REQ-COMP-014`: First-class `Resumption k a b e`, `with`, and `resume` MUST lower through validated selective CPS to explicit runtime contexts; deep one-shot, shallow one-shot, and admissible bounded multi-shot modes MUST preserve evaluation order, source origins, handler depth, ownership, effects, consumption, and failure behavior through loaded BEAM.
- `REQ-COMP-015`: Control-flow optimization MUST be proof-gated, independently disableable for equivalence testing, conservative for open or unresolved capabilities, and source-diagnostic preserving.
