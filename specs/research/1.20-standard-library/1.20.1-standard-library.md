...
- Don't conflict with other operators in Catena

Here's the updated document with the changes:

# Catena Standard Library: Category Theory Foundations

## Overview

This document presents the core category theory abstractions for Catena's standard library, designed to bridge mathematical elegance with BEAM's practical runtime. Each concept includes beginner-friendly explanations, implementations, and law verification using the `laws` testing framework.
...
```catena
-- File: lib/catena/category/core.tps
