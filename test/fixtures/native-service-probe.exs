...
{:ok, description} =
  Package.describe(:nif, payloads, scheduler: :dirty_cpu, timeout_ms: 50, max_work_units: 1)
...
policy = %{
  ...
  unsafe_acknowledgements: Package.obligations(:nif)
}
...
|> case do
  :ok -> :ok
  _ -> raise "destructor did not close"
end
...
IO.puts(
  "finite bits, dirty scheduler, rejection, double close, GC fallback, bounded timeout verified"
)
