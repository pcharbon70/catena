defmodule Catena.C027EntryPointsTest do
  ...
  test "0.1.23 is an exact registered revision with predecessors pinned" do
    ...
    assert {:ok, %{selection: %{language_revision: "0.1.68"}}} = Catena.decode_source_text("")
  ...
  defp boom_binary do
    forms = [
      {:attribute, 0, :module, C027Boom},
      {:attribute, 0, :export, [{:boom, 0}]},
      {:function, 0, :boom, 0,
       [
         {:clause, 0, [], [],
          [
            {:call, 0, {:remote, 0, {:atom, 0, :erlang}, {:atom, 0, :error}},
             [:erl_parse.abstract(ArgumentError.exception("entry trap"))]}
          ]}
       ]}
    ]

    {:ok, module, binary, _} = Catena.OTP.Compiler.compile(forms)
    {module, binary}
  end
