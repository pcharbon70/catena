...
-module(catena_prop_process_tests).
...
spawn_test_process_with_name_test() ->
    Fun = fun() -> receive after infinity -> ok end end,
    Proc = catena_prop_process:spawn_test_process(Fun, #{name => test_proc_name}),
...
spawn_test_process_without_link_test() ->
    Fun = fun() -> receive after infinity -> ok end end,
    Proc = catena_prop_process:spawn_test_process(Fun, #{link => false}),
...
with_process_auto_cleanup_test() ->
    ...
    catena_prop_process:with_process(Fun, TestFun),
...
kill_process_terminates_forcibly_test() ->
    ...
    Pid = Proc#test_process.pid,
    ?assert(is_process_alive(Pid)),
    catena_prop_process:kill_process(Proc),
    ?assertNot(is_process_alive(Pid)),
    ok.
...
process_info_safe_alive_process_test() ->
    ...
    Proc = catena_prop_process:spawn_test_process(Fun),
    Pid = Proc#test_process.pid,
    Info = catena_prop_process:process_info_safe(Pid, [message_queue_len]),
...
process_info_safe_dead_process_test() ->
    ...
    Info = catena_prop_process:process_info_safe(FakePid, [memory]),
    ?assertMatch({error, _}, Info),
    ok.

get_state_returns_genserver_state_test() ->
    {ok, Pid} = gen_server:start_link(?MODULE, #{count => 3}, []),
    ?assertEqual(#{count => 3}, catena_prop_process:get_state(Pid)),
    gen_server:stop(Pid),
    ok.
...
message_queue_empty_test() ->
    ...
    Proc = catena_prop_process:spawn_test_process(Fun),
    Pid = Proc#test_process.pid,
    {ok, QLen} = catena_prop_process:message_queue(Pid),
    ?assertEqual(0, QLen),
    catena_prop_process:stop_process(Proc),
    ok.
...
process_memory_returns_number_test() ->
    ...
    Proc = catena_prop_process:spawn_test_process(Fun),
    Pid = Proc#test_process.pid,
    {ok, Mem} = catena_prop_process:process_memory(Pid),
...
full_state_returns_complete_info_test() ->
    ...
    Proc = catena_prop_process:spawn_test_process(Fun),
    Pid = Proc#test_process.pid,
    State = catena_prop_process:full_state(Pid),
...
gen_pid_returns_pid_test() ->
    Gen = catena_prop_process:gen_pid(),
    Seed = #seed{state = 42},
    {Pid, _Seed} = Gen(Seed),
    ?assert(is_pid(Pid)),
    ok.
...
gen_ref_returns_unique_reference_test() ->
    Gen = catena_prop_process:gen_ref(),
    Seed1 = #seed{state = 42},
    Seed2 = #seed{state = 43},
    {Ref1, _} = Gen(Seed1),
    {Ref2, _} = Gen(Seed2),
...
gen_node_returns_node_name_test() ->
    Gen = catena_prop_process:gen_node(),
    Seed = #seed{state = 42},
    {Node, _} = Gen(Seed),
    ?assert(is_atom(Node)),
    ok.
...
gen_registered_name_returns_atom_test() ->
    Gen = catena_prop_process:gen_registered_name(),
    Seed = #seed{state = 42},
    {Name, _} = Gen(Seed),
    ?assert(is_atom(Name)),
    ok.
...
gen_timeout_returns_timeout_value_test() ->
    Gen = catena_prop_process:gen_timeout(),
    Seed = #seed{state = 42},
    {Timeout1, _} = Gen(Seed),
...
gen_exit_reason_returns_term_test() ->
    Gen = catena_prop_process:gen_exit_reason(),
    Seed = #seed{state = 42},
    {Reason, _} = Gen(Seed),
...
assert_alive_fails_for_dead_process_test() ->
    FakePid = list_to_pid("<0.9999.0>"),
    Result = catena_prop_process:assert_alive(FakePid),
    ?assertMatch({error, {process_not_alive, _}}, Result),
    ok.

assert_dead_passes_for_dead_process_test() ->
    FakePid = list_to_pid("<0.9999.0>"),
    ?assertEqual(ok, catena_prop_process:assert_dead(FakePid)),
    ok.

assert_dead_fails_for_live_process_test() ->
    Fun = fun() -> receive after infinity -> ok end end,
    Proc = catena_prop_process:spawn_test_process(Fun),
    Result = catena_prop_process:assert_dead(Proc),
...
assert_exit_reason_tracks_shutdown_test() ->
    ...
    Pid = Proc#test_process.pid,
    catena_prop_process:stop_process(Proc),
    ?assertEqual(ok, catena_prop_process:assert_exit_reason(Pid, shutdown)),
    ok.
...
assert_exit_reason_detects_mismatch_test() ->
    ...
    Pid = Proc#test_process.pid,
    catena_prop_process:kill_process(Proc),
    Result = catena_prop_process:assert_exit_reason(Pid, shutdown),
...
assert_no_messages_passes_for_empty_queue_test() ->
    ...
    Proc = catena_prop_process:spawn_test_process(Fun),
    Pid = Proc#test_process.pid,
    ?assertEqual(ok, catena_prop_process:assert_no_messages(Pid)),
    catena_prop_process:stop_process(Proc),
    ok.
...
assert_no_messages_fails_for_nonempty_queue_test() ->
    ...
    Pid = Proc#test_process.pid,
    Pid ! test_message,
    Result = catena_prop_process:assert_no_messages(Pid),
...
assert_message_count_passes_for_correct_count_test() ->
    ...
    Pid ! msg1,
    Pid ! msg2,
    ?assertEqual(ok, catena_prop_process:assert_message_count(Pid, 2)),
    catena_prop_process:stop_process(Proc),
    ok.
...
assert_message_count_fails_for_wrong_count_test() ->
    ...
    Proc = catena_prop_process:spawn_test_process(Fun),
    Pid = Proc#test_process.pid,
    Result = catena_prop_process:assert_message_count(Pid, 5),
    ?assertMatch({error, {wrong_message_count, 5, _}}, Result),
    catena_prop_process:stop_process(Proc),
    ok.

assert_registered_fails_for_unregistered_name_test() ->
    Result = catena_prop_process:assert_registered(nonexistent_process),
...
process_lifecycle_full_test() ->
    ...
    %% 2. Verify alive
    ?assertEqual(ok, catena_prop_process:assert_alive(Pid)),
    ...
    %% 5. Stop process
    catena_prop_process:stop_process(Pid),

    %% 6. Verify dead
    ?assertEqual(ok, catena_prop_process:assert_dead(Pid)),
    ok.

cleanup_processes_handles_multiple_processes_test() ->
    %% Spawn multiple processes
    Procs = [catena_prop_process:spawn_test_process(
    ...
    %% Cleanup
    ?assertEqual(ok, catena_prop_process:cleanup_processes()),
