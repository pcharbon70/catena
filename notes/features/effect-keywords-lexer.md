...
The lexer implementation (`catena_lexer.xrl`) successfully handles:
...
We will extend the existing `catena_lexer.xrl` leex definition file by adding five new keyword rules in the Rules section. This is a minimal, non-invasive change that follows the established pattern for keyword recognition.

### High-Level Approach

1. **Add keyword rules** to `catena_lexer.xrl` in the Rules section after existing keywords
...
│   ├── compiler/
│   │   ├── lexer/
│   │   │   ├── catena_lexer.xrl       # MODIFY: Add 5 keyword rules
│   │   │   └── catena_lexer_gen.erl   # AUTO-GENERATED: Will be regenerated
└── test/
    └── catena_lexer_tests.erl         # MODIFY: Add test cases for new keywords
```

Note: `catena_lexer_gen.erl` is auto-generated by leex from `catena_lexer.xrl` and should not be edited manually.

### Specific Code Changes

#### Change 1: Add Effect Keywords to Rules Section

**File**: `/home/ducky/code/catena/src/compiler/lexer/catena_lexer.xrl`
...
This will automatically invoke leex to regenerate `catena_lexer_gen.erl` from the modified `catena_lexer.xrl`.

**Expected Output**:
- Successful compilation with no warnings
- Updated `src/compiler/lexer/catena_lexer_gen.erl` (auto-generated)
- Updated `ebin/catena_lexer_gen.beam` (compiled bytecode)
...
**Actions**:
1. Open `/home/ducky/code/catena/src/compiler/lexer/catena_lexer.xrl`
...
3. Check that `src/compiler/lexer/catena_lexer_gen.erl` was regenerated (check timestamp)
...
**Actions**:
1. Open `/home/ducky/code/catena/test/catena_lexer_tests.erl`
2. Add all unit test cases from Testing Strategy section above
3. Run tests: `./rebar3 eunit --module=catena_lexer_tests`
...
2. Manually test each keyword:
   ```erlang
   {ok, T1} = catena_lexer:tokenize("effect").
   {ok, T2} = catena_lexer:tokenize("operation").
   {ok, T3} = catena_lexer:tokenize("perform").
   {ok, T4} = catena_lexer:tokenize("try").
   {ok, T5} = catena_lexer:tokenize("with").
   ```
3. Test complete effect syntax:
   ```erlang
   {ok, Tokens} = catena_lexer:tokenize("effect FileIO { operation read }").
   ```
...
**Actions**:
1. Update any relevant documentation that lists Catena keywords
...
- Not consistent with existing Catena keyword approach
- Harder to maintain and debug

**Decision**: Rejected - Not worth the complexity for minimal benefit. All other Catena keywords are unconditional.
...
This feature planning document provides a comprehensive blueprint for integrating algebraic effect system keywords into the Catena lexer. The implementation is:
...
- **Lexer Module**: `/home/ducky/code/catena/src/compiler/lexer/catena_lexer.erl`
- **Generated Lexer**: `/home/ducky/code/catena/src/compiler/lexer/catena_lexer_gen.erl`
- **Test Suite**: `/home/ducky/code/catena/test/catena_lexer_tests.erl`
