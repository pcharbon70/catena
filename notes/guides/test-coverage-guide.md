...
This guide explains how to use test coverage reporting in the Catena compiler project.

## Overview

Test coverage measures how much of the source code is executed during testing. The Catena project uses Erlang's built-in `cover` tool to track line coverage for all type system modules.
...
```bash
# On Linux
xdg-open _build/test/cover/catena_types.html

# On macOS
open _build/test/cover/catena_types.html

# On Windows
start _build/test/cover/catena_types.html
```
...
- **Core modules** (catena_types, catena_type_subst, catena_type_scheme): ≥95%
- **Utility modules** (catena_type_pp, catena_type_env): ≥90%
- **Error handling** (catena_type_error): ≥80%
- **Integration modules** (catena_compiler_utils): ≥75%

## Excluded Modules

The following modules are excluded from coverage reporting:

- **catena_lexer_gen**: Auto-generated by leex
- **catena_parser**: Auto-generated by yecc
...
```bash
./scripts/run_coverage.sh catena_types_tests
```
...
% In the shell:
1> cover:start().
2> cover:compile_beam(catena_types).
3> catena_types_tests:test().
4> cover:analyse_to_file(catena_types, "_build/test/cover/catena_types.html", [html]).
5> cover:analyse(catena_types, coverage, line).
```
...
**Example**: If `catena_types:tvariant/1` has low coverage:

```erlang
% In test/compiler/types/catena_types_tests.erl
test_variant_construction() ->
    % Test single constructor
    V1 = catena_types:tvariant([{none, []}]),
    ?assertMatch({tvariant, [{none, []}]}, V1),

    % Test multiple constructors
    IntType = catena_types:tcon(integer),
    V2 = catena_types:tvariant([
        {some, [IntType]},
        {none, []}
    ]),
...
3. Include in PR description:
   ...
   ## Test Coverage
   - catena_types: 96% (+2%)
   - catena_type_subst: 94% (unchanged)
   - catena_type_scheme: 91% (+5%)
   ```
