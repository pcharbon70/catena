defmodule Catena.Type.Parser do
  ...
  @spec parse_scheme(map(), String.t()) :: Scheme.t()
  def parse_scheme(signature, path) when is_map(signature) do
    ...
    type = parse(Map.get(signature, "type"), ids, path <> ".type")
  ...
  def parse_scheme(_signature, path), do: fail("T012", "signature must be an object", path)

  @spec parse(map(), map(), String.t()) :: Type.t()
  def parse(%{"tag" => "integer"}, _variables, _path), do: :integer
  def parse(%{"tag" => "boolean"}, _variables, _path), do: :boolean

  def parse(%{"tag" => "variable", "name" => name}, variables, path) do
  ...
  def parse(
    ...
    {:function, parse(parameter, variables, path <> ".parameter"),
     parse(result, variables, path <> ".result")}
  end

  def parse(%{"tag" => "tuple", "elements" => elements}, variables, path)
      when is_list(elements) do
    {:tuple,
     elements
     |> Enum.with_index()
     |> Enum.map(fn {element, index} ->
       parse(element, variables, "#{path}.elements[#{index}]")
     end)}
  end

  def parse(_, _variables, path), do: fail("T012", "unsupported or malformed type", path)
