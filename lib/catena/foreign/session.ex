defmodule Catena.Foreign.Session do
  @moduledoc false
  use GenServer
  alias Catena.Foreign.{Adapter, Budget, Callback, Codec, Control, Descriptor}
  ...
  def start(owner, grants, limits, options) do
    ...
    if match?({:ok, _}, Descriptor.selection(options)) and is_list(grants) and
         Budget.valid?(limits) and is_integer(maximum) and maximum > 0 and
         is_boolean(callbacks) and Enum.all?(grants, &(Descriptor.verify(&1) == :ok)) do
      GenServer.start(
        __MODULE__,
        {owner, grants, limits, maximum, callbacks, Keyword.fetch!(options, :release_token)}
      )
    else
      {:error, :invalid_foreign_scope_setup}
    end
  end

  @impl true
  def init({owner, grants, limits, maximum, callbacks, release_token}) do
  ...
  defp arguments(state, schemas, values)
    ...
    data =
      ...
      |> Enum.reject(fn {schema, _} -> match?({:callback, _, _}, schema) end)
